"""CLI para inspeccionar y editar la sección ZENODO de settings.toml.
...
escribir el prefijo 'ZENODO.' en cada get/set, y el wizard solo pregunta por
estos campos (no por GENERATE/GENERATE_TOY/HUGGINGFACE). No gestiona ningún
YAML propio — 'zenodo prepare'/'upload'/'check-readiness'/'release' siempre
leen la plantilla incluida en el proyecto (templates/Zenodo.yaml.j2),
rellenada con estos mismos valores.
"""
import typer
...
console = Console()
app     = typer.Typer(help="Inspecciona o edita la sección ZENODO de settings.toml.")
...
def config_show() -> None:
    """Muestra los valores actuales de ZENODO en settings.toml."""
    settings = load_settings()
    section_model = settings.ZENODO
...
@app.command("get")
def config_get(
    field: str = typer.Argument(..., help="Nombre del campo, ej. environment (sin prefijo ZENODO.)."),
) -> None:
...
def config_set(
    assignment: str = typer.Argument(
        ...,
        help=(
            "Asignación CAMPO=VALOR (ej. environment=production), o CAMPO a secas para "
            "campos secretos — pide el valor con entrada oculta en vez de "
            "escribirlo en claro en el comando."
        ),
    ),
) -> None:
    """Cambia el valor de un campo de ZENODO y lo guarda en settings.toml."""
    settings = load_settings()
    section_model = settings.ZENODO
    ...
    if "=" in assignment:
        ...
        if not _is_secret_field(section_model, field):
            raise typer.BadParameter("Formato esperado: CAMPO=VALOR (ej. environment=production).")
    ...
    secret = _is_secret_field(section_model, field)
    console.print(f"[green]✔  {SECTION}.{field} = {_format_value(getattr(new_settings.ZENODO, field), secret)}[/green]")


@app.command("wizard")
def config_wizard() -> None:
    """Asistente interactivo: pregunta solo por los campos de ZENODO."""
    settings = load_settings()

    console.print(
        "[bold]Asistente de configuración de Zenodo[/bold]\n"
        "Pulsa Enter en cualquier pregunta para mantener el valor actual."
    )

    new_settings = settings
    section_model = new_settings.ZENODO
    for field in type(section_model).model_fields:
    ...
    changed = False
    old_model = settings.ZENODO
    new_model = new_settings.ZENODO
