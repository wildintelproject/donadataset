"""CLI para inspeccionar y editar la sección GBIF de settings.toml.
...
escribir el prefijo 'GBIF.' en cada get/set, y el wizard solo pregunta por
estos campos (no por GENERATE/HUGGINGFACE/ZENODO/B2SHARE). No gestiona
ningún YAML/JSON propio — 'gbif prepare' genera el datapackage.json del
paquete Camtrap DP directamente a partir de estos mismos valores.
"""
import typer
...
console = Console()
app     = typer.Typer(help="Inspecciona o edita la sección GBIF de settings.toml.")
...
def config_show() -> None:
    """Muestra los valores actuales de GBIF en settings.toml (los secretos aparecen enmascarados)."""
    settings = load_settings()
    section_model = settings.GBIF
...
@app.command("get")
def config_get(
    field: str = typer.Argument(..., help="Nombre del campo, ej. contact_email (sin prefijo GBIF.)."),
) -> None:
...
def config_set(
    assignment: str = typer.Argument(
        ...,
        help=(
            "Asignación CAMPO=VALOR (ej. contact_email=you@example.org), o CAMPO a secas "
            "para campos secretos (ej. username, password) — pide el valor con entrada "
            "oculta en vez de escribirlo en claro en el comando."
        ),
    ),
) -> None:
    """Cambia el valor de un campo de GBIF y lo guarda en settings.toml."""
    settings = load_settings()
    section_model = settings.GBIF
    ...
    if "=" in assignment:
        ...
        if not _is_secret_field(section_model, field):
            raise typer.BadParameter("Formato esperado: CAMPO=VALOR (ej. contact_email=you@example.org).")
    ...
    secret = _is_secret_field(section_model, field)
    console.print(f"[green]✔  {SECTION}.{field} = {_format_value(getattr(new_settings.GBIF, field), secret)}[/green]")


@app.command("wizard")
def config_wizard() -> None:
    """Asistente interactivo: pregunta solo por los campos de GBIF."""
    settings = load_settings()

    console.print(
        "[bold]Asistente de configuración de GBIF[/bold]\n"
        "Pulsa Enter en cualquier pregunta para mantener el valor actual."
    )

    new_settings = settings
    section_model = new_settings.GBIF
    for field in type(section_model).model_fields:
    ...
    changed = False
    old_model = settings.GBIF
    new_model = new_settings.GBIF
