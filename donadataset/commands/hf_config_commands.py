"""CLI para inspeccionar y editar la sección HUGGINGFACE de settings.toml.
...
escribir el prefijo 'HUGGINGFACE.' en cada get/set, y el wizard solo
pregunta por estos campos (no por GENERATE/GENERATE_TOY/ZENODO). No gestiona
ningún YAML propio — 'huggingface prepare' siempre lee la plantilla incluida
en el proyecto (templates/hfh/HuggingFaceHub.yaml.j2), rellenada con estos
mismos valores.
"""
import typer
...
console = Console()
app     = typer.Typer(help="Inspecciona o edita la sección HUGGINGFACE de settings.toml.")
...
def config_show() -> None:
    """Muestra los valores actuales de HUGGINGFACE en settings.toml (los secretos aparecen enmascarados)."""
    settings = load_settings()
    section_model = settings.HUGGINGFACE
...
@app.command("get")
def config_get(
    field: str = typer.Argument(..., help="Nombre del campo, ej. repo_id (sin prefijo HUGGINGFACE.)."),
) -> None:
...
def config_set(
    assignment: str = typer.Argument(
        ...,
        help=(
            "Asignación CAMPO=VALOR (ej. repo_id=miuser/donadataset), o CAMPO a secas para "
            "campos secretos (ej. token) — pide el valor con entrada oculta en vez de "
            "escribirlo en claro en el comando."
        ),
    ),
) -> None:
    """Cambia el valor de un campo de HUGGINGFACE y lo guarda en settings.toml."""
    settings = load_settings()
    section_model = settings.HUGGINGFACE
    ...
    if "=" in assignment:
        ...
        if not _is_secret_field(section_model, field):
            raise typer.BadParameter("Formato esperado: CAMPO=VALOR (ej. repo_id=miuser/donadataset).")
    ...
    secret = _is_secret_field(section_model, field)
    console.print(f"[green]✔  {SECTION}.{field} = {_format_value(getattr(new_settings.HUGGINGFACE, field), secret)}[/green]")


@app.command("wizard")
def config_wizard() -> None:
    """Asistente interactivo: pregunta solo por los campos de HUGGINGFACE."""
    settings = load_settings()

    console.print(
        "[bold]Asistente de configuración de HuggingFace Hub[/bold]\n"
        "Pulsa Enter en cualquier pregunta para mantener el valor actual."
    )

    new_settings = settings
    section_model = new_settings.HUGGINGFACE
    for field in type(section_model).model_fields:
    ...
    changed = False
    old_model = settings.HUGGINGFACE
    new_model = new_settings.HUGGINGFACE
